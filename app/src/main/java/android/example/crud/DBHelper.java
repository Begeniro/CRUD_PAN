...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void updateBuku(int id, String judul, int harga) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(KEY_JUDUL, judul);
        values.put(KEY_HARGA, harga);

        db.update(TABLE_BUKU, values, KEY_ID_BUKU + " = ?", new String[]{String.valueOf(id)});
    }

    //method untuk delete buku
    public void delete(int id){
        SQLiteDatabase db=this.getWritableDatabase();
        String deleteQuery="DELETE FROM "+TABLE_BUKU+" WHERE "+KEY_ID_BUKU+" = '"+ id+"'";
